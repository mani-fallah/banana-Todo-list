        ...
        if(!task)
          alert("please fill out this input")
            ...
            taskdeleteEl.addEventListener('click',()=>{
                ...
                {
                    if (allToDos[i] === document.querySelector('.text').value)
                    {
                        allToDos = allToDos.filter(function(item) {
                            return item !== document.querySelector('.text').value
                        })
                    }
                }
...
function displayList(todos)
{
    ...
    if(todos[i])
    {
        ...
        taskdeleteEl.addEventListener('click',()=>{
            for (let i = 0;i< todos.length;i++)
            {
                allToDos = todos.filter(function(item) {
                    return item !== document.querySelector('.text').value
                })
            }
            localStorage.setItem("todos",JSON.stringify(allToDos))
